// Minna Lesson 01 i18n single-script stable 14.4.8
// Fixes remaining strings: Login, Core Vocabulary title, Nice to meet you greeting, Correct feedback.
(function(){
  const KEY='minna_ui_lang';
  const pairs={
    'AI互动学习App｜第1课':'AI Interactive Learning App | Lesson 1','《みんなの日本語 初級Ⅰ》第1课':'Minna no Nihongo Beginner I Lesson 1','名词句・自我介绍':'Noun sentences / self-introduction','寒暄':'greeting',
    'G Google登录':'Google Login','G oogle登录':'Google Login','Google登录':'Google Login','登录':'Login','已登录':'Signed in','云端':'Cloud','已连接':'Connected','学习会自动保存':'learning will be saved automatically','已保存':'Saved','分数':'Score','等级':'Level','完成':'Done','错题':'Mistakes','课程首页':'Home','手动保存':'Manual Save','读取':'Load','退出':'Log out','学习地图':'Learning Map','使用说明':'Guide','本课重点':'Lesson Focus',
    ...
    '答对了':'Correct!','答对了！':'Correct!','答对了！错题已复习。':'Correct! The mistake has been reviewed.','错题已复习':'The mistake has been reviewed'
  };
  ...
  function toEn(s){
    ...
    out=out.replace(/Login[:：]?\s*□?已?Login[:：]?/g,'Login:').replace(/Login[:：]\s*已Login[:：]/g,'Login:').replace(/Login[:：]\s*Signed inLogin[:：]/g,'Login:');
    out=out.replace(/Login[:：]\s*([^\s<]*@[^\s<]*)/g,'Login: $1');
    out=out.replace(/Cloud[:：]?\s*supabase/g,'Cloud: Supabase');
    out=out.replace(/Cloud[:：].*学习会自动保存/g,'Cloud: Supabase connected. Learning will be saved automatically.');
    out=out.replace(/Core Vocabulary[｜|]\s*选择题/g,'Core Vocabulary | Choice');
    out=out.replace(/Nice to meet you\s*寒暄/g,'Nice to meet you / greeting').replace(/Nice to meet yougreeting/g,'Nice to meet you / greeting');
  ...
  function walk(root){
    if(!root)return;
    const NF=(root.ownerDocument||document).defaultView.NodeFilter;
    const walker=(root.ownerDocument||document).createTreeWalker(root,NF.SHOW_TEXT,{acceptNode(node){const p=node.parentElement;if(!p||['SCRIPT','STYLE','TEXTAREA','INPUT','SELECT','OPTION'].includes(p.tagName))return NF.FILTER_REJECT;return node.nodeValue.trim()?NF.FILTER_ACCEPT:NF.FILTER_REJECT;}});
