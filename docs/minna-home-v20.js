// Minna Home v20.3
// Learner-first entry for the unified JSON player, content audit, review hub, and direct lesson access.
(function(){
  const VERSION='20.3';
  ...
  const copy={
    ...
    continueLesson:{zh:'继续第{n}课',en:'Continue Lesson {n}'},currentLesson:{zh:'当前建议：第{n}课',en:'Recommended: Lesson {n}'},wrongbook:{zh:'错题本',en:'Mistakes'},review:{zh:'总复习',en:'Review Hub'},
    ...
    todayTask:{zh:'今日任务',en:"Today's Task"},todayTaskDesc:{zh:'完成第{n}课 Mastery 或清空本课错题。',en:'Complete Lesson {n} Mastery or clear its mistakes.'},currentProgress:{zh:'当前课进度',en:'Current Progress'},checkInYes:{zh:'已有学习记录',en:'Learning record found'},checkInNo:{zh:'开始一课即可打卡',en:'Start a lesson to check in'},streak:{zh:'连续学习',en:'Learning Streak'},streakDays:{zh:'{count} 天',en:'{count} days'},
    report:{zh:'学习报告概览',en:'Learning Report'},reportDesc:{zh:'根据本机与云端学习记录生成基础报告。',en:'A basic report based on local and cloud learning records.'},completion:{zh:'总完成率',en:'Overall Completion'},mistakeTotal:{zh:'错题总数',en:'Total Mistakes'},nextGoal:{zh:'下一目标',en:'Next Goal'},nextGoalText:{zh:'完成第{n}课并解锁下一课',en:'Complete Lesson {n} and unlock the next one'},badgeTitle:{zh:'阶段徽章',en:'Stage Badges'},badgeLocked:{zh:'继续学习解锁',en:'Keep learning to unlock'},badgeReady:{zh:'已获得',en:'Earned'},stageBadge1:{zh:'初级 I 前半',en:'Beginner I First Half'},stageBadge2:{zh:'初级 I 完成',en:'Beginner I Complete'},stageBadge3:{zh:'初级 II 前半',en:'Beginner II First Half'},stageBadge4:{zh:'全课完成',en:'All Lessons Complete'},
    ...
    stageMastered:{zh:'{done}/{total} 掌握',en:'{done}/{total} mastered'},stageComplete:{zh:'阶段完成',en:'Stage Complete'},noMatch:{zh:'没有匹配课程。',en:'No matching lessons.'},unlocked:{zh:'已解锁',en:'Unlocked'},locked:{zh:'未解锁 · 可预览',en:'Locked · Preview'},current:{zh:'当前课',en:'Current'},privileged:{zh:'已开放',en:'Open'},progress:{zh:'进度 {percent}%',en:'Progress {percent}%'},wrong:{zh:'错题 {wrong}',en:'Mistakes {wrong}'},lockedHint:{zh:'请先完成第{n}课，或进入预览模式。',en:'Finish Lesson {n} first, or open preview mode.'},medalMastered:{zh:'🏅 Mastery',en:'🏅 Mastery'},medalCurrent:{zh:'🔥 当前挑战',en:'🔥 Current Challenge'},medalUnlocked:{zh:'✨ 可学习',en:'✨ Available'},medalLocked:{zh:'🔒 待解锁',en:'🔒 Locked'}
  };
  ...
  const text=(key,vars)=>{let value=pick(copy[key]);if(vars)Object.keys(vars).forEach(k=>{value=String(value).split('{'+k+'}').join(vars[k])});return value};
  const esc=s=>String(s==null?'':s).replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  const readJson=key=>{try{return JSON.parse(localStorage.getItem(key)||'null')}catch(e){return null}};
  function installEnhanceStyle(){
    if(document.getElementById('minnaHomeEnhanceV203'))return;
    const style=document.createElement('style');style.id='minnaHomeEnhanceV203';
    style.textContent=`.lesson.current{animation:minnaPulse 1.8s ease-in-out infinite}.lesson.mastered:after{content:'✓';position:absolute;right:12px;top:10px;background:#22c55e;color:#052e16;border-radius:999px;width:26px;height:26px;display:flex;align-items:center;justify-content:center;font-weight:1000}.lesson{position:relative}.medalLine{display:flex!important;gap:6px;flex-wrap:wrap;margin-top:8px}.medal{display:inline-flex!important;width:auto!important;background:#f1f5f9;border-radius:999px;padding:2px 8px;color:#334155!important;font-weight:1000;font-size:11px}.lesson.current .medal{background:#dcfce7;color:#166534!important}.lesson.mastered .medal{background:#fef3c7;color:#92400e!important}.reportGrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px}.reportCard{background:#fff;border:1px solid var(--line);border-radius:8px;padding:14px;box-shadow:0 8px 22px rgba(15,23,42,.05)}.reportCard b{display:block;font-size:26px;line-height:1.1}.badgeGrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px}.stageMedal{background:#fff;border:1px solid var(--line);border-radius:8px;padding:14px;box-shadow:0 8px 22px rgba(15,23,42,.05)}.stageMedal.earned{border-color:#86efac;background:#f0fdf4}.stageMedal b{display:block;font-size:22px}@keyframes minnaPulse{0%,100%{box-shadow:0 8px 22px rgba(34,197,94,.18)}50%{box-shadow:0 16px 38px rgba(34,197,94,.38);transform:translateY(-2px)}}`;
  ...
  const lessonId=n=>'minna_lesson_'+pad(n);
  const cloudState=n=>{const row=cloudProgress[lessonId(n)];return row&&row.progress?row.progress:null};
  ...
  const canAccess=n=>roleState.bypassLessonLock||n===1||progressOf(n-1).passed;
  ...
  const lessonHref=(l,access)=>access?l.url:`${l.url}&mode=preview`;
  ...
  const roleModeLabel=()=>roleState.bypassLessonLock?text('roleOpen'):text('rolePath');
  function recordDateValue(p){const s=p.state||{};return String(s.updated_at||s.last_answer_at||s.lastAt||s.lastStudyDate||s.completed_at||s.saved_at||'')}
  function dateKeyFromRecord(p){const raw=recordDateValue(p);if(!raw)return '';const d=new Date(raw);if(!isNaN(d.getTime()))return d.toISOString().slice(0,10);return raw.slice(0,10)}
  const todayKey=()=>new Date().toISOString().slice(0,10);
  function streakFromDates(enriched){
    const dates=new Set(enriched.map(l=>dateKeyFromRecord(l.progress)).filter(Boolean));
    let cursor=new Date(todayKey()+'T00:00:00Z'),count=0;
    for(let i=0;i<365;i++){const key=cursor.toISOString().slice(0,10);if(!dates.has(key))break;count++;cursor.setUTCDate(cursor.getUTCDate()-1)}
    return count;
  }
  ...
  function dashboard(){
    ...
    const mistakes=enriched.reduce((sum,l)=>sum+(Number(l.progress.wrong)||0),0);
    ...
    const todayDone=enriched.some(l=>dateKeyFromRecord(l.progress)===todayKey());
    return {enriched,mastered,records,mistakes,current,resume,todayDone,streak:streakFromDates(enriched),completion:Math.round(mastered/50*100)};
  }
  function stageBadges(mastered){
    const items=[[13,'stageBadge1'],[25,'stageBadge2'],[38,'stageBadge3'],[50,'stageBadge4']];
    return `<div class="badgeGrid">${items.map(([need,key])=>{const earned=mastered>=need;return `<div class="stageMedal ${earned?'earned':''}"><b>${earned?'🏆':'🔒'}</b><strong>${esc(text(key))}</strong><small>${esc(earned?text('badgeReady'):text('badgeLocked'))}</small></div>`}).join('')}</div>`;
  }
  function render(){
    installEnhanceStyle();
    const data=dashboard();
    document.title=`${text('title')} v${VERSION}`;
    $('app').innerHTML=`<header class="hero"><div class="wrap heroGrid"><div class="heroCopy"><div class="heroMeta"><span class="badge">Minna AI Learning System v${VERSION}</span><span class="roleBadge">${esc(roleLabel())} · ${esc(roleModeLabel())}</span><span id="langMount"></span></div><h1>${esc(text('title'))}</h1><p>${esc(text('subtitle'))}</p><div class="heroActions"><a class="primary" href="${data.resume.url}" data-track="${data.resume.n}">${esc(text('continueLesson',{n:data.resume.n}))}</a><a class="ghostHero" href="${lessonUrl(data.current.n)}" data-track="${data.current.n}">${esc(text('currentLesson',{n:data.current.n}))}</a><a class="lightHero" href="./minna-review.html?v=${VERSION}">${esc(text('review'))}</a><a class="lightHero" href="./minna-wrongbook.html?v=${VERSION}">${esc(text('wrongbook'))}</a></div></div><div class="heroStats"><div><b>${data.mastered}</b><span>${esc(text('mastered'))}</span></div><div><b>${data.records}</b><span>${esc(text('records'))}</span></div><div><b>${data.completion}%</b><span>${esc(text('completion'))}</span></div></div></div></header><main class="wrap"><section class="panel overviewPanel"><div><h2>${esc(text('today'))}</h2><p>${esc(text('todayDesc'))}</p></div><div class="actionStrip"><a class="primary" href="${data.resume.url}" data-track="${data.resume.n}">${esc(text('continue'))}</a><a class="ghost" href="./minna-review.html?v=${VERSION}">${esc(text('review'))}</a><a class="ghost" href="./minna-content-audit.html?v=${VERSION}">${esc(text('audit'))}</a><a class="ghost" href="./minna-system-design.html?v=${VERSION}">${esc(text('design'))}</a><a class="light" href="./minna-admin.html?v=${VERSION}">${esc(text('admin'))}</a><a class="light" href="./minna-user-manual.html?v=${VERSION}">${esc(text('manual'))}</a></div></section><section class="panel statusPanel"><h2>${esc(text('todayTask'))}</h2><div class="statusGrid"><div><b>${esc(text('todayTask'))}</b><span>${esc(text('todayTaskDesc',{n:data.current.n}))}</span></div><div><b>${data.current.progress.percent}%</b><span>${esc(text('currentProgress'))}</span></div><div><b>${data.todayDone?'✓':'—'}</b><span>${esc(data.todayDone?text('checkInYes'):text('checkInNo'))}</span></div><div><b>${esc(text('streakDays',{count:data.streak}))}</b><span>${esc(text('streak'))}</span></div></div></section><section class="panel"><div class="sectionHead"><div><h2>${esc(text('report'))}</h2><p class="small">${esc(text('reportDesc'))}</p></div></div><div class="reportGrid"><div class="reportCard"><b>${data.completion}%</b><span>${esc(text('completion'))}</span></div><div class="reportCard"><b>${data.mistakes}</b><span>${esc(text('mistakeTotal'))}</span></div><div class="reportCard"><b>${data.current.n}</b><span>${esc(text('nextGoal'))}: ${esc(text('nextGoalText',{n:data.current.n}))}</span></div><div class="reportCard"><b>${data.streak}</b><span>${esc(text('streak'))}</span></div></div></section><section class="panel"><h2>${esc(text('badgeTitle'))}</h2>${stageBadges(data.mastered)}</section><section class="panel"><div class="sectionHead"><div><h2>${esc(text('path'))}</h2><p class="small">${esc(text('pathDesc'))}</p></div><div class="segmented" role="tablist" aria-label="${esc(text('path'))}"><button data-filter="all">${esc(text('all'))}</button><button data-filter="migrated">${esc(text('migrated'))}</button><button data-filter="seed">${esc(text('seed'))}</button><button data-filter="unfinished">${esc(text('unfinished'))}</button><button data-filter="mistakes">${esc(text('mistakesOnly'))}</button><button data-filter="mastered">${esc(text('masteredOnly'))}</button><button data-filter="available">${esc(text('availableOnly'))}</button></div></div><div class="filters"><input id="searchBox" value="${esc(query)}" placeholder="${esc(text('search'))}"><select id="filterBox"><option value="all">${esc(text('allCourses'))}</option><option value="migrated">${esc(text('migrated'))}</option><option value="seed">${esc(text('seed'))}</option><option value="unfinished">${esc(text('unfinished'))}</option><option value="mistakes">${esc(text('mistakesOnly'))}</option><option value="mastered">${esc(text('masteredOnly'))}</option><option value="available">${esc(text('availableOnly'))}</option></select></div><div id="lessonGrid"></div></section><section class="panel statusPanel"><h2>${esc(text('status'))}</h2><div class="statusGrid"><div><b>${esc(text('player'))}</b><span>${esc(text('playerDesc'))}</span></div><div><b>${esc(text('practice'))}</b><span>${esc(text('practiceDesc'))}</span></div><div><b>${esc(text('fallback'))}</b><span>${esc(text('fallbackDesc'))}</span></div></div></section></main><footer class="wrap footer">docs/minna-index.html · v${VERSION}</footer>`;
  ...
  function renderGrid(){
    ...
    const html=stages.map(([title,from,to])=>{
      ...
      return `<div class="stageBlock"><div class="stageTitle"><h3>${esc(pick(title))}</h3><span class="badge2">${esc(complete?text('stageComplete'):text('stageMastered',{done,total:rows.length}))}</span></div><div class="lessonGrid">${rows.map(card).join('')}</div></div>`;
    }).join('');
  ...
  function card(l){
    ...
    const medal=p.passed?text('medalMastered'):current?text('medalCurrent'):access?text('medalUnlocked'):text('medalLocked');
    ...
    return `<a class="lesson ${status}" href="${lessonHref(l,access)}" data-track="${l.n}"><b>${esc(lessonName)}</b><span>${esc(pick(l.topic))}</span><small>${esc(label)} · ${esc(l.tag)}</small><div class="progressLine"><i style="width:${p.percent}%"></i></div><small>${esc(text('progress',{percent:p.percent}))}${esc(wrong)}</small><small class="medalLine"><i class="medal">${esc(medal)}</i></small>${hint}</a>`;
  }
