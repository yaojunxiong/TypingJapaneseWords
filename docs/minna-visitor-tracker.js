...
(function(){
  ...
  function recordStudyDay(){
    try{
      const key='minna_study_days';
      const today=todayKey();
      let days=JSON.parse(localStorage.getItem(key)||'[]');
      if(!Array.isArray(days)) days=[];
      if(!days.includes(today)) days.push(today);
      days=Array.from(new Set(days)).sort().slice(-120);
      localStorage.setItem(key,JSON.stringify(days));
      return days;
    }catch(e){return [todayKey()]}
  }
  function studyStats(){
    const days=recordStudyDay();
    ...
    for(let i=0;i<365;i++){
      ...
      if(set.has(k)) streak++; else break;
    }
    const week=[];
  ...
  function renderLeaderboardTop3(panel){
    ...
    var rows=parseLeaderboardRows(panel);var anchor=panel.querySelector('.tableWrap');var html='';
  ...
  function enhanceLeaderboard(){
    ...
    if(!panel.dataset.enhanced){panel.dataset.enhanced='1';panel.classList.add('leaderCollapsed');var btn=document.createElement('button');btn.className='light leaderToggle';btn.textContent='展开完整排行榜';btn.onclick=function(){panel.classList.toggle('leaderCollapsed');btn.textContent=panel.classList.contains('leaderCollapsed')?'展开完整排行榜':'收起排行榜'};var head=panel.querySelector('.leaderHead')||panel;head.appendChild(btn)}
  ...
  function enhanceStages(){
    ...
    stages.forEach(function(st){
      if(!st.dataset.foldReady){st.dataset.foldReady='1';var isCurrent=st.classList.contains('currentStage');var isDone=st.classList.contains('doneStage');if(!isCurrent)st.classList.add('collapsed');var btn=document.createElement('button');btn.className='stageCollapseBtn';btn.textContent=isCurrent?'收起本阶段':(isDone?'展开已完成阶段':'展开本阶段');btn.onclick=function(e){e.preventDefault();st.classList.toggle('collapsed');btn.textContent=st.classList.contains('collapsed')?(isDone?'展开已完成阶段':'展开本阶段'):'收起本阶段'};var head=st.querySelector('.stageHead');if(head)head.appendChild(btn)}
    });
    focusCurrentPath();
  }
  function installStickyBar(){
    var n=currentLessonFromContinue();
    var bar=document.getElementById('stickyStudyBar');
    if(!bar){bar=document.createElement('nav');bar.id='stickyStudyBar';bar.className='stickyStudyBar';document.body.appendChild(bar)}
    var wrong=sumWrongCount();
  ...
  function start(){
    recordStudyDay();
    track();
    var observer=new MutationObserver(function(){clearTimeout(window.__minnaHomeEnhanceTimer);window.__minnaHomeEnhanceTimer=setTimeout(enhanceHome,180)});
