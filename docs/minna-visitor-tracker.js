...
(function(){
  ...
  function injectHomeStyle(){
    ...
    var s=document.createElement('style');s.id='minnaHomeEnhancerStyle';s.textContent=`
      ...
      .dashMain b{font-size:22px}.dashItem b{font-size:24px;display:block}.dashActions,.goalBtns,.quickBtns{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;align-items:center}
      .weekGoalBar{height:9px;background:#e2e8f0;border-radius:999px;overflow:hidden;margin:8px 0}.weekGoalBar span{display:block;height:100%;background:linear-gradient(90deg,#22c55e,#f59e0b)}
      .focusModeBtn,.goalBtn,.jumpBtn{border:0;border-radius:12px;padding:8px 10px;font-weight:1000;cursor:pointer}.focusModeBtn{background:#0f172a;color:white}.goalBtn,.jumpBtn{background:#e2e8f0;color:#0f172a}.goalBtn.active{background:#22c55e;color:#052e16}.quickSelect{width:100%;border:1px solid #e2e8f0;border-radius:14px;padding:10px 12px;background:white;font-weight:800}
      .taskGrid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-top:10px}.taskList{margin:8px 0 0 18px;padding:0}.taskList li{margin:4px 0}.wrongPriority{background:linear-gradient(135deg,#fff7ed,#fef2f2);border-color:#fed7aa}.wrongZero{background:linear-gradient(135deg,#f0fdf4,#ecfeff);border-color:#bbf7d0}.taskTitle{font-weight:1000;font-size:17px}.taskNum{font-size:28px;font-weight:1000}.streakHot{background:linear-gradient(135deg,#fff7ed,#fef3c7);border-color:#f59e0b}
      ...
      @media(max-width:1100px){.taskGrid{grid-template-columns:1fr 1fr}.taskCard:last-child{grid-column:1/-1}}
  ...
  function lessonUrl(n,suffix){n=Math.min(50,Math.max(1,Number(n)||1));return './minna-no-nihongo-lesson-'+String(n).padStart(2,'0')+'.html?v=13.8'+(suffix||'')}
  ...
  function start(){recordStudyDay();track();var observer=new MutationObserver(function(){clearTimeout(window.__minnaHomeEnhanceTimer);window.__minnaHomeEnhanceTimer=setTimeout(enhanceHome,180)});observer.observe(document.body,{childList:true,subtree:true,characterData:true})}
