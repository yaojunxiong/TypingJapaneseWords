...
(function(){
  ...
  function injectHomeStyle(){
    ...
    var s=document.createElement('style');s.id='minnaHomeEnhancerStyle';s.textContent=`
      ...
      @media(max-width:760px){.dashGrid,.taskGrid{grid-template-columns:1fr 1fr}.dashMain{grid-column:1/-1}.taskCard{grid-column:1/-1}}
  ...
  function sumWrongCount(){
    var nodes=Array.prototype.slice.call(document.querySelectorAll('.node')).filter(n=>!/总复习/.test(n.textContent||''));
    var sum=0;nodes.forEach(function(node){var m=(node.textContent||'').match(/错题\s*(\d+)/);if(m)sum+=Number(m[1])||0});return sum;
  }
  ...
  function buildTasks(n,totalWrong,currentWrong,donePages,achText){
    ...
    if(currentWrong>0){tasks.push('重点处理当前课错题 '+currentWrong+' 个')}
    else if(achText==='未完成'&&n>25){tasks.push('抽空挑战 1–25课总复习，拿前半册成就')}
    else{tasks.push('朗读例句 5 句，提高听说反应速度')}
    return tasks;
  }
  function installDashboard(){
    ...
    dash.innerHTML='<h2>📌 今日学习仪表盘</h2><div class="dashGrid"><div class="dashMain"><div class="small">今天建议学习</div><b>第'+n+'课</b><p class="small">优先完成当前课，再继续解锁下一课。</p><div class="dashActions"><a class="btn primary" href="./minna-no-nihongo-lesson-'+String(n).padStart(2,'0')+'.html?v=13.4">继续学习</a><a class="btn light" href="./minna-review-01-25.html">1–25课总复习</a></div></div><div class="dashItem"><span class="small">已掌握</span><b>'+done+'/50</b></div><div class="dashItem"><span class="small">总分</span><b>'+score+'</b></div><div class="dashItem"><span class="small">前半册成就</span><b style="font-size:20px">'+achText+'</b><span class="small">有记录课程 '+records+'</span></div></div><div class="taskGrid"><div class="taskCard '+wrongClass+'"><div class="taskTitle">🎯 错题优先</div><div class="taskNum">'+totalWrong+'</div><p class="small">'+wrongMsg+' 当前课错题：'+currentWrong+' 个。</p><div class="dashActions"><a class="btn '+(totalWrong>0?'primary':'light')+'" href="./minna-no-nihongo-lesson-'+String(n).padStart(2,'0')+'.html?v=13.4#wrong">复习错题</a><a class="btn light" href="./minna-no-nihongo-lesson-'+String(n).padStart(2,'0')+'.html?v=13.4">继续当前课</a></div></div><div class="taskCard"><div class="taskTitle">✅ 今日任务</div><ol class="taskList"><li>'+esc(tasks[0])+'</li><li>'+esc(tasks[1])+'</li><li>'+esc(tasks[2])+'</li></ol><p class="small">建议 10–15 分钟完成，不求多，但要连续。</p></div></div>';
  }
  ...
  function start(){
    track();
    var observer=new MutationObserver(function(){clearTimeout(window.__minnaHomeEnhanceTimer);window.__minnaHomeEnhanceTimer=setTimeout(enhanceHome,120)});
