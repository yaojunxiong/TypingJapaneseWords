# Full-Site Practice Page Quality Check

Generated: 2026-05-31T13:52:13.831Z

## Validation Rules

- **Multiple-choice**: options ≥ 4, exactly 1 correct
- **Order (n ≥ 3 fragments)**: options ≥ 4, exactly 1 correct
- **Order (n = 2 fragments)**: options ≥ 2 (max unique permutations), exactly 1 correct

## Overall Summary

- **200** stages checked (50 lessons × 4 stages)
- **200** ✅ PASS / **0** ❌ FAIL
- **1747** total questions generated
- **0** stages with insufficient-options issues
- **0** stages with !=1 correct issues
- **0** stages with 0 questions (empty state)
- **0** stages with shuffle anomaly (all correct at index 0)

## Per-Stage Detail

| Lsn | Stage | Status | Qs | Opt<4 | Corr!=1 | Corr@0 | Flags |
|---|---|---|---|---|---|---|---|
| 01 | vocab | ✅ | 17 | ✅ | ✅ | 5 | — |
| 01 | grammar | ✅ | 6 | ✅ | ✅ | 1 | — |
| 01 | examples | ✅ | 6 | ✅ | ✅ | 2 | — |
| 01 | quiz | ✅ | 9 | ✅ | ✅ | 3 | — |
| 02 | vocab | ✅ | 33 | ✅ | ✅ | 9 | — |
| 02 | grammar | ✅ | 8 | ✅ | ✅ | 3 | — |
| 02 | examples | ✅ | 5 | ✅ | ✅ | 1 | — |
| 02 | quiz | ✅ | 4 | ✅ | ✅ | 2 | — |
| 03 | vocab | ✅ | 16 | ✅ | ✅ | 5 | — |
| 03 | grammar | ✅ | 8 | ✅ | ✅ | 1 | — |
| 03 | examples | ✅ | 5 | ✅ | ✅ | 2 | — |
| 03 | quiz | ✅ | 4 | ✅ | ✅ | 0 | — |
| 04 | vocab | ✅ | 16 | ✅ | ✅ | 5 | — |
| 04 | grammar | ✅ | 8 | ✅ | ✅ | 3 | — |
| 04 | examples | ✅ | 5 | ✅ | ✅ | 2 | — |
| 04 | quiz | ✅ | 4 | ✅ | ✅ | 1 | — |
| 05 | vocab | ✅ | 16 | ✅ | ✅ | 4 | — |
| 05 | grammar | ✅ | 8 | ✅ | ✅ | 4 | — |
| 05 | examples | ✅ | 5 | ✅ | ✅ | 1 | — |
| 05 | quiz | ✅ | 4 | ✅ | ✅ | 1 | — |
| 06 | vocab | ✅ | 16 | ✅ | ✅ | 5 | — |
| 06 | grammar | ✅ | 8 | ✅ | ✅ | 4 | — |
| 06 | examples | ✅ | 5 | ✅ | ✅ | 2 | — |
| 06 | quiz | ✅ | 4 | ✅ | ✅ | 1 | — |
| 07 | vocab | ✅ | 16 | ✅ | ✅ | 4 | — |
| 07 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 07 | examples | ✅ | 5 | ✅ | ✅ | 2 | — |
| 07 | quiz | ✅ | 4 | ✅ | ✅ | 1 | — |
| 08 | vocab | ✅ | 16 | ✅ | ✅ | 4 | — |
| 08 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 08 | examples | ✅ | 5 | ✅ | ✅ | 2 | — |
| 08 | quiz | ✅ | 4 | ✅ | ✅ | 1 | — |
| 09 | vocab | ✅ | 16 | ✅ | ✅ | 3 | — |
| 09 | grammar | ✅ | 8 | ✅ | ✅ | 1 | — |
| 09 | examples | ✅ | 5 | ✅ | ✅ | 0 | — |
| 09 | quiz | ✅ | 4 | ✅ | ✅ | 2 | — |
| 10 | vocab | ✅ | 16 | ✅ | ✅ | 3 | — |
| 10 | grammar | ✅ | 8 | ✅ | ✅ | 0 | — |
| 10 | examples | ✅ | 5 | ✅ | ✅ | 2 | — |
| 10 | quiz | ✅ | 4 | ✅ | ✅ | 1 | — |
| 11 | vocab | ✅ | 16 | ✅ | ✅ | 4 | — |
| 11 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 11 | examples | ✅ | 5 | ✅ | ✅ | 1 | — |
| 11 | quiz | ✅ | 4 | ✅ | ✅ | 1 | — |
| 12 | vocab | ✅ | 16 | ✅ | ✅ | 4 | — |
| 12 | grammar | ✅ | 8 | ✅ | ✅ | 1 | — |
| 12 | examples | ✅ | 5 | ✅ | ✅ | 1 | — |
| 12 | quiz | ✅ | 4 | ✅ | ✅ | 0 | — |
| 13 | vocab | ✅ | 16 | ✅ | ✅ | 3 | — |
| 13 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 13 | examples | ✅ | 5 | ✅ | ✅ | 1 | — |
| 13 | quiz | ✅ | 4 | ✅ | ✅ | 0 | — |
| 14 | vocab | ✅ | 16 | ✅ | ✅ | 4 | — |
| 14 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 14 | examples | ✅ | 5 | ✅ | ✅ | 2 | — |
| 14 | quiz | ✅ | 4 | ✅ | ✅ | 0 | — |
| 15 | vocab | ✅ | 16 | ✅ | ✅ | 5 | — |
| 15 | grammar | ✅ | 8 | ✅ | ✅ | 1 | — |
| 15 | examples | ✅ | 5 | ✅ | ✅ | 1 | — |
| 15 | quiz | ✅ | 4 | ✅ | ✅ | 0 | — |
| 16 | vocab | ✅ | 16 | ✅ | ✅ | 4 | — |
| 16 | grammar | ✅ | 8 | ✅ | ✅ | 0 | — |
| 16 | examples | ✅ | 5 | ✅ | ✅ | 1 | — |
| 16 | quiz | ✅ | 4 | ✅ | ✅ | 0 | — |
| 17 | vocab | ✅ | 16 | ✅ | ✅ | 3 | — |
| 17 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 17 | examples | ✅ | 5 | ✅ | ✅ | 1 | — |
| 17 | quiz | ✅ | 4 | ✅ | ✅ | 0 | — |
| 18 | vocab | ✅ | 16 | ✅ | ✅ | 4 | — |
| 18 | grammar | ✅ | 8 | ✅ | ✅ | 4 | — |
| 18 | examples | ✅ | 5 | ✅ | ✅ | 1 | — |
| 18 | quiz | ✅ | 4 | ✅ | ✅ | 1 | — |
| 19 | vocab | ✅ | 16 | ✅ | ✅ | 3 | — |
| 19 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 19 | examples | ✅ | 5 | ✅ | ✅ | 1 | — |
| 19 | quiz | ✅ | 4 | ✅ | ✅ | 2 | — |
| 20 | vocab | ✅ | 16 | ✅ | ✅ | 4 | — |
| 20 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 20 | examples | ✅ | 5 | ✅ | ✅ | 2 | — |
| 20 | quiz | ✅ | 4 | ✅ | ✅ | 1 | — |
| 21 | vocab | ✅ | 16 | ✅ | ✅ | 4 | — |
| 21 | grammar | ✅ | 8 | ✅ | ✅ | 3 | — |
| 21 | examples | ✅ | 5 | ✅ | ✅ | 1 | — |
| 21 | quiz | ✅ | 4 | ✅ | ✅ | 2 | — |
| 22 | vocab | ✅ | 16 | ✅ | ✅ | 3 | — |
| 22 | grammar | ✅ | 8 | ✅ | ✅ | 0 | — |
| 22 | examples | ✅ | 5 | ✅ | ✅ | 2 | — |
| 22 | quiz | ✅ | 4 | ✅ | ✅ | 1 | — |
| 23 | vocab | ✅ | 16 | ✅ | ✅ | 4 | — |
| 23 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 23 | examples | ✅ | 5 | ✅ | ✅ | 1 | — |
| 23 | quiz | ✅ | 4 | ✅ | ✅ | 0 | — |
| 24 | vocab | ✅ | 16 | ✅ | ✅ | 4 | — |
| 24 | grammar | ✅ | 8 | ✅ | ✅ | 1 | — |
| 24 | examples | ✅ | 5 | ✅ | ✅ | 1 | — |
| 24 | quiz | ✅ | 4 | ✅ | ✅ | 1 | — |
| 25 | vocab | ✅ | 16 | ✅ | ✅ | 3 | — |
| 25 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 25 | examples | ✅ | 5 | ✅ | ✅ | 1 | — |
| 25 | quiz | ✅ | 4 | ✅ | ✅ | 1 | — |
| 26 | vocab | ✅ | 10 | ✅ | ✅ | 3 | — |
| 26 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 26 | examples | ✅ | 8 | ✅ | ✅ | 2 | — |
| 26 | quiz | ✅ | 10 | ✅ | ✅ | 3 | — |
| 27 | vocab | ✅ | 10 | ✅ | ✅ | 4 | — |
| 27 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 27 | examples | ✅ | 8 | ✅ | ✅ | 1 | — |
| 27 | quiz | ✅ | 10 | ✅ | ✅ | 3 | — |
| 28 | vocab | ✅ | 10 | ✅ | ✅ | 3 | — |
| 28 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 28 | examples | ✅ | 8 | ✅ | ✅ | 1 | — |
| 28 | quiz | ✅ | 10 | ✅ | ✅ | 3 | — |
| 29 | vocab | ✅ | 10 | ✅ | ✅ | 2 | — |
| 29 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 29 | examples | ✅ | 8 | ✅ | ✅ | 3 | — |
| 29 | quiz | ✅ | 10 | ✅ | ✅ | 2 | — |
| 30 | vocab | ✅ | 10 | ✅ | ✅ | 2 | — |
| 30 | grammar | ✅ | 8 | ✅ | ✅ | 3 | — |
| 30 | examples | ✅ | 8 | ✅ | ✅ | 0 | — |
| 30 | quiz | ✅ | 10 | ✅ | ✅ | 3 | — |
| 31 | vocab | ✅ | 10 | ✅ | ✅ | 1 | — |
| 31 | grammar | ✅ | 8 | ✅ | ✅ | 1 | — |
| 31 | examples | ✅ | 8 | ✅ | ✅ | 3 | — |
| 31 | quiz | ✅ | 10 | ✅ | ✅ | 4 | — |
| 32 | vocab | ✅ | 10 | ✅ | ✅ | 4 | — |
| 32 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 32 | examples | ✅ | 8 | ✅ | ✅ | 2 | — |
| 32 | quiz | ✅ | 10 | ✅ | ✅ | 3 | — |
| 33 | vocab | ✅ | 10 | ✅ | ✅ | 3 | — |
| 33 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 33 | examples | ✅ | 8 | ✅ | ✅ | 1 | — |
| 33 | quiz | ✅ | 10 | ✅ | ✅ | 2 | — |
| 34 | vocab | ✅ | 10 | ✅ | ✅ | 1 | — |
| 34 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 34 | examples | ✅ | 8 | ✅ | ✅ | 2 | — |
| 34 | quiz | ✅ | 10 | ✅ | ✅ | 3 | — |
| 35 | vocab | ✅ | 10 | ✅ | ✅ | 3 | — |
| 35 | grammar | ✅ | 8 | ✅ | ✅ | 3 | — |
| 35 | examples | ✅ | 8 | ✅ | ✅ | 2 | — |
| 35 | quiz | ✅ | 10 | ✅ | ✅ | 4 | — |
| 36 | vocab | ✅ | 10 | ✅ | ✅ | 2 | — |
| 36 | grammar | ✅ | 8 | ✅ | ✅ | 0 | — |
| 36 | examples | ✅ | 8 | ✅ | ✅ | 2 | — |
| 36 | quiz | ✅ | 10 | ✅ | ✅ | 2 | — |
| 37 | vocab | ✅ | 10 | ✅ | ✅ | 3 | — |
| 37 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 37 | examples | ✅ | 8 | ✅ | ✅ | 2 | — |
| 37 | quiz | ✅ | 10 | ✅ | ✅ | 2 | — |
| 38 | vocab | ✅ | 10 | ✅ | ✅ | 3 | — |
| 38 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 38 | examples | ✅ | 8 | ✅ | ✅ | 2 | — |
| 38 | quiz | ✅ | 10 | ✅ | ✅ | 2 | — |
| 39 | vocab | ✅ | 10 | ✅ | ✅ | 2 | — |
| 39 | grammar | ✅ | 8 | ✅ | ✅ | 1 | — |
| 39 | examples | ✅ | 8 | ✅ | ✅ | 1 | — |
| 39 | quiz | ✅ | 10 | ✅ | ✅ | 3 | — |
| 40 | vocab | ✅ | 10 | ✅ | ✅ | 1 | — |
| 40 | grammar | ✅ | 8 | ✅ | ✅ | 4 | — |
| 40 | examples | ✅ | 8 | ✅ | ✅ | 2 | — |
| 40 | quiz | ✅ | 10 | ✅ | ✅ | 0 | — |
| 41 | vocab | ✅ | 10 | ✅ | ✅ | 2 | — |
| 41 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 41 | examples | ✅ | 8 | ✅ | ✅ | 2 | — |
| 41 | quiz | ✅ | 10 | ✅ | ✅ | 2 | — |
| 42 | vocab | ✅ | 10 | ✅ | ✅ | 2 | — |
| 42 | grammar | ✅ | 8 | ✅ | ✅ | 1 | — |
| 42 | examples | ✅ | 8 | ✅ | ✅ | 3 | — |
| 42 | quiz | ✅ | 10 | ✅ | ✅ | 1 | — |
| 43 | vocab | ✅ | 10 | ✅ | ✅ | 1 | — |
| 43 | grammar | ✅ | 8 | ✅ | ✅ | 3 | — |
| 43 | examples | ✅ | 8 | ✅ | ✅ | 2 | — |
| 43 | quiz | ✅ | 10 | ✅ | ✅ | 2 | — |
| 44 | vocab | ✅ | 10 | ✅ | ✅ | 2 | — |
| 44 | grammar | ✅ | 8 | ✅ | ✅ | 1 | — |
| 44 | examples | ✅ | 8 | ✅ | ✅ | 3 | — |
| 44 | quiz | ✅ | 10 | ✅ | ✅ | 3 | — |
| 45 | vocab | ✅ | 10 | ✅ | ✅ | 4 | — |
| 45 | grammar | ✅ | 8 | ✅ | ✅ | 0 | — |
| 45 | examples | ✅ | 8 | ✅ | ✅ | 2 | — |
| 45 | quiz | ✅ | 10 | ✅ | ✅ | 2 | — |
| 46 | vocab | ✅ | 10 | ✅ | ✅ | 3 | — |
| 46 | grammar | ✅ | 8 | ✅ | ✅ | 4 | — |
| 46 | examples | ✅ | 8 | ✅ | ✅ | 2 | — |
| 46 | quiz | ✅ | 10 | ✅ | ✅ | 3 | — |
| 47 | vocab | ✅ | 10 | ✅ | ✅ | 3 | — |
| 47 | grammar | ✅ | 8 | ✅ | ✅ | 2 | — |
| 47 | examples | ✅ | 8 | ✅ | ✅ | 2 | — |
| 47 | quiz | ✅ | 10 | ✅ | ✅ | 2 | — |
| 48 | vocab | ✅ | 10 | ✅ | ✅ | 2 | — |
| 48 | grammar | ✅ | 8 | ✅ | ✅ | 1 | — |
| 48 | examples | ✅ | 8 | ✅ | ✅ | 2 | — |
| 48 | quiz | ✅ | 10 | ✅ | ✅ | 2 | — |
| 49 | vocab | ✅ | 10 | ✅ | ✅ | 1 | — |
| 49 | grammar | ✅ | 8 | ✅ | ✅ | 1 | — |
| 49 | examples | ✅ | 8 | ✅ | ✅ | 3 | — |
| 49 | quiz | ✅ | 10 | ✅ | ✅ | 2 | — |
| 50 | vocab | ✅ | 10 | ✅ | ✅ | 3 | — |
| 50 | grammar | ✅ | 8 | ✅ | ✅ | 4 | — |
| 50 | examples | ✅ | 8 | ✅ | ✅ | 1 | — |
| 50 | quiz | ✅ | 10 | ✅ | ✅ | 3 | — |

## 🎉 All Stages Passed

Every stage generates valid questions:
- Choice questions: ≥4 options, exactly 1 correct
- Order questions (n≥3): ≥4 options, exactly 1 correct
- Order questions (n=2): ≥2 options, exactly 1 correct

## Shuffle Verification

Correct answer position distribution (after `seededShuffle`):

| Pos | Stages |
|---|---|
- **0%** at index 0: 16 stages
- **1–30%** at index 0: 143 stages
- **31–70%** at index 0: 41 stages
- **71–99%** at index 0: 0 stages
- **100%** at index 0: 0 stages (anomaly — all flagged)

## Quality Risks (Requiring Manual Review)

The following issues cannot be auto-detected and require human verification:

1. **Option text quality**: All options exist and have text, but distractors may be semantically unrelated or too easy to eliminate.
2. **Question relevance**: The auto-generated practice questions (vocab/examples) always use the format `「XX」的含义是？` — this may not cover all useful exercise types.
3. **LangText vs flat string inconsistency**: v2 quiz (lessons 39–50) stores option text as flat strings; v1 uses `{zh,en,jp}` objects. The page handles both, but the data inconsistency remains.
4. **Order-type questions**: With 2 fragments only 2 unique permutations exist; with 3+ fragments we generate up to 6 and keep 4. The current permutation strategies (swap, reverse, rotate) may not cover all interesting orderings.
5. **Explanations**: Not all questions have explanations — the page gracefully handles missing explanations, but learners lose feedback.
6. **Cross-lesson consistency**: Lessons 26–50 have a `mistakes` section type that the audit ignores. This section does not generate practice, but learners may expect it.
