...
export default async function AdminLessonDetailPage({
  ...
  const backHref = String(back || '').startsWith('/admin') ? String(back) : '/?audit=1'
  ...
  return (
    <main>
      ...
      <section className="card">
        ...
        <p className="small">
          <Link href={`//lessons/${prevNo}?back=${backParam}`}>{tr(lang, '上一课', 'Previous lesson')}</Link>
          {' · '}
          <Link href={`//lessons/${nextNo}?back=${backParam}`}>{tr(lang, '下一课', 'Next lesson')}</Link>
        </p>
