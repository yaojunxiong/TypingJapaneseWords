      ...
      <section className="card">
        ...
        <div style={{ display: 'grid', gap: 8 }}>
          {knowledgeLinks.map((kl) => (
            <Link
              key={kl.file}
              href={`//knowledge-base?file=${kl.file}`}
              className="pillLink"
      ...
      <table className="table" style={{ minWidth: 360 }}>
        <tbody>
          {DEFINITIONS.map((key) => {
            ...
            return (
              <tr key={key}>
                ...
                <td>
                  <Link href={`//workflows?definition_key=${key}`} style={{ fontSize: 12 }}>{tr(lang, '查看', 'View')}</Link>
                </td>
              </tr>
            )
          })}
          <tr>
            ...
            <td><Link href="/workflows" style={{ fontSize: 12 }}>{tr(lang, '查看', 'View')}</Link></td>
          </tr>
        </tbody>
      </table>
      <p className="small" style={{ marginTop: 8 }}>
        <Link href="/workflows">{tr(lang, '前往审批流程管理', 'Go to Approval Workflow Management')}</Link>
      </p>
    </section>
  )
}
