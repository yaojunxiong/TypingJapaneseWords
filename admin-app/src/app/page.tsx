...
export default async function AdminPage({
  ...
  const backParam = encodeURIComponent(`/${stateQuery ? `?${stateQuery}` : ''}`)
  const backHref = `/${stateQuery ? `?${stateQuery}` : ''}`
    ...
    <>
      ...
      <main style={{ background: '#f8fafc', paddingBottom: 'calc(140px + env(safe-area-inset-bottom, 0px))' }}>
        ...
        <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', maxWidth: 960, margin: '0 auto 20px' }}>
          ...
          />
          <Link href="/workflows" style={{ textDecoration: 'none', color: 'inherit' }}>
            <StatCard
              icon="⏳"
          ...
          <Link href="/workflows?definition_key=study_visitor" style={{ textDecoration: 'none', color: 'inherit' }}>
            <StatCard
              icon="👤"
          ...
          <Link href="/workflows?definition_key=logged_in_first_visit" style={{ textDecoration: 'none', color: 'inherit' }}>
            <StatCard
              icon="🔑"
          ...
          <Link href="/workflows?definition_key=membership_application" style={{ textDecoration: 'none', color: 'inherit' }}>
            <StatCard
              icon="📋"
          ...
          <Link href="/email-logs" style={{ textDecoration: 'none', color: 'inherit' }}>
            <StatCard
              icon="📧"
          ...
          <Link href="/email-logs?status=failed" style={{ textDecoration: 'none', color: 'inherit' }}>
            <StatCard
              icon="❌"
          ...
          <>
            ...
            <section className="card" style={{ maxWidth: 960, margin: '0 auto 14px' }}>
              ...
              <div style={{ display: 'grid', gap: 8 }}>
                <Link href="/knowledge-base?file=admin-legacy-branch-extraction-plan.md" className="pillLink" style={{ textDecoration: 'none', display: 'block' }}>
                  📄 {tr(lang, '旧分支后台能力提取计划', 'Legacy Admin Extraction Plan')}
                </Link>
                <Link href="/knowledge-base?file=admin-system-deep-trace-audit.md" className="pillLink" style={{ textDecoration: 'none', display: 'block' }}>
                  📄 {tr(lang, '全项目后台能力深度追溯', 'Full Admin System Deep Trace')}
                </Link>
                <Link href="/knowledge-base?file=admin-system-current-state-audit.md" className="pillLink" style={{ textDecoration: 'none', display: 'block' }}>
            ...
            <section className="card" style={{ maxWidth: 960, margin: '0 auto 14px' }}>
              ...
              <p>
                <Link className="btn" href="/?audit=1">{tr(lang, '一键运行 Audit', 'Run One-Click Audit')}</Link>
              </p>
        ...
        {runAudit ? (
          <div style={{ maxWidth: 960, margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
              <Link className="btn ghost" href="/">{tr(lang, '← 返回仪表盘', '← Back to Dashboard')}</Link>
            ...
            <section className="card" style={{ maxWidth: 960, margin: '0 auto 14px' }}>
              ...
              <form method="get" style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <Link className="btn ghost" href="/?audit=1">{tr(lang, '重置', 'Reset')}</Link>
                ...
                {query ? (
                  <Link
                    className="btn ghost"
                    href={`//export.csv?q=${encodeURIComponent(query)}&section=${encodeURIComponent(sectionFilter)}${lessonFilter ? `&lesson=${lessonFilter}` : ''}&sort=${sortBy}`}
                  >
              ...
              <table className="table" style={{ minWidth: 860 }}>
                ...
                <tbody>
                  {rows.map((row) => (
                    <tr key={row.no}>
                      <td>
                        <Link href={`//lessons/${row.no}?back=${backParam}`}>{String(row.no).padStart(2, '0')}</Link>
                      </td>
                      <td>{row.sections}</td>
                      <td>{row.vocab}</td>
                      <td>{row.grammar}</td>
              ...
              <section className="card" style={{ maxWidth: 960, margin: '0 auto 14px' }}>
                ...
                {!searchHits.length ? (
                  ...
                  pagedHits.map((hit, idx) => (
                    <div key={`${hit.lessonNo}-${hit.itemId}-${idx}`} style={{ marginBottom: 10 }}>
                      <b>
                        <Link href={`//lessons/${hit.lessonNo}?back=${backParam}#${anchorIdForItem(hit.itemId)}`}>{tr(lang, '第', 'Lesson ')}{hit.lessonNo}{tr(lang, '课', '')}</Link>
                      </b>
                ...
                {searchHits.length > SEARCH_PAGE_SIZE ? (
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 8 }}>
                    {safePage > 1 ? (
                      <Link className="btn ghost" href={`/?audit=1&q=${encodeURIComponent(query)}&section=${encodeURIComponent(sectionFilter)}${lessonFilter ? `&lesson=${lessonFilter}` : ''}&sort=${sortBy}&page=${safePage - 1}`}>
                        {tr(lang, '上一页', 'Prev')}
                      </Link>
                    ) : null}
